...
### We will create our Subscription and Resources, 
then go over Failed Authentication and Log Observation, 
and finally Azure Active Directory Overview (Users, Groups, and Identity Access Management)
...
- A mini firewall will be configured for our virtual machine to allow all traffic in.
- This will make our firewall highly enticing to attack and allow threat actors such as hackers, bots, and attackers to attempt to get into our virtual machine. 
...
- Based on the traffic coming into the network we can see the priority categorised in Azure based on the set rules/protocols. 
- Create an Inbound Security Rule, Any, Name it "DangerAllInBound" 
...
- No it didn't because we also need to remote in and configure the firewall setting within the VM. 
- Remote Into the VM
...
- Select "Mixed Mode"
- This is important because, with Windows Authentication Mode, we will only be able to log in with an online account, whereas, with a mixed mode, we can log in online and locally into the SQL Server.
...
- Now, Intentionally enter the wrong username and password. This will register as a failed login attempt. 
...
- Here we can see the failed login attempt and the reason.
- That concludes the first lab. 
...
- We are creating an attack VM with the goal to be from a different region so it looks like a threat actor is attacking our enticing windows-VM. 
...
```Be sure to KEEP your resources``` we will continue with the same RG & VM. 
